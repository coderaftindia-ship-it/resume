<div class="row">
    ...
    <div class="col-lg-4 col-md-6">
        <div class="form-group">
            ...
            {{ Form::text('start_date',\Carbon\Carbon::parse($education->start_date??null)->format('m/d/Y'), ['class' => 'form-control datepicker','required','placeholder'=>__('messages.select_start_date'),'id'=>'startDate', 'data-date-end-date'=>"0d",'data-date-start-date'=>\Carbon\Carbon::parse(getUserDate())->format('m/d/Y'),'autocomplete'=>'off']) }}
        </div>
    </div>
        <div class="col-lg-4 col-md-6 {{$education->currently_studying_here?'d-none':''}} endDateDiv">
            <div class="form-group">
                {{ Form::label('end_date', __('messages.end_date').':') }}
                {{ Form::text('end_date',!empty($education->end_date) ? \Carbon\Carbon::parse($education->end_date)->format('m/d/Y'): null, ['class' => 'form-control datepicker end-date', 'id'=>'endDate', 'placeholder'=>__('messages.select_end_date'),'data-date-end-date'=>"0d",'data-date-start-date'=>\Carbon\Carbon::parse(getUserDate())->format('m/d/Y'),'autocomplete'=>'off']) }}
            </div>
        </div>
